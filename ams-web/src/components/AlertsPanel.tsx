...
const KR_ALERT_REASON: Record<string, string> = {
  ...
  command_send_failed:
    '이 계정으로 보낸 명령이 전송 재시도 한도까지 실패했습니다. 전달은 대기로 되돌아가고 활성·비활성 전환은 폐기되므로, 그대로 두면 아무 일도 일어나지 않습니다. 서버가 온라인인지 확인하고, 특히 그 서버에 에이전트가 아직 설치되지 않았을 가능성부터 확인한 뒤 배정 목록에서 같은 작업을 다시 실행하세요.',
