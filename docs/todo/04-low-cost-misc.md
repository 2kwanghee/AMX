...
> 상태: **완료 (2026-08-09)** — 실행 항목 전부 머지: E1·E2(PR #30) · B2·B3(PR #29) · G33+docstring(PR #31) ·
> G27·G28(PR #32) · stale 행 정정(C2/E3/A1/C1, 앞 순위에서 처리). 아래 원문은 이력으로 보존.
>
> **잔여 G nits 선별 판정 (이월 — 착수는 사용자 결정)**:
> - 설계 판단 필요: G22(claim-before-write, 별도 설계 명시됨) · G30+G12+C1잔존창(앱레벨 ack 인프라 통합) · G35 후반(switch_now 경보 시맨틱)
> - 외부 노출/상용 전 필수(내재화 위협 낮음): G29(rate limit·길이 상한) · G34(force 감사 기록)
> - 소규모 인접 하드닝 후보(저비용): G6(alert ack 레이스) · G8(FindByEmail 비결정) · G13(quarantine 상태 경보) · G23(F3 문서 과장) · G31(UpdateBaseline CAS)
> - 기록만으로 충분: G1~G5·G7·G9~G11·G15·G18·G21(각 행 참조) · G24(프로세스 규칙, 적용 중)
