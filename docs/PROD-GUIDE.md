...
한도가 찬 계정을 배급처·대여중·충전소로 순환시키는 컨트롤러다(개념·사양은
`AMX-DESIGN.md` §5.8). 컨트롤러 자체는 항상 돌지만(관측), 서버를 **자동 모드**로 켜기
전에는 명령을 내지 않는다. 조율 변수는 dev.env에 둔다(전부 선택, 미설정 시 기본값).

```
AMX_POOL_WINDOW_HIGH_PCT=80              # 계정 창 고사용 경보 임계 (%)
AMX_POOL_OBSERVATION_GRACE_MINUTES=15    # 충전소 복귀 관측 유예 (분)
AMX_POOL_WINDOW_STALE_MINUTES=30         # 관측을 낡았다고 보는 경계 (분)
AMX_POOL_CHAIN_STEP_TIMEOUT_MINUTES=10   # 체인 한 단계 시간 초과 (분)
AMX_POOL_CHAIN_MAX_MINUTES=60            # 체인 전체 상한 (분)
AMX_POOL_MAX_CONCURRENT_CHAINS=3         # 테넌트 동시 체인 상한
AMX_POOL_EVENT_RETENTION_DAYS=90         # pool_events 보존 (일)
```

**업그레이드 시 마이그레이션.** 기동 스크립트(`deploy/fullstack-run.sh`)가 매 기동에
`alembic upgrade head`를 돌리므로 계정 풀 테이블(0028~0031)은 자동 적용된다. 다만 0031이
"서버당 활성 체인 1개"를 부분 유니크 인덱스로 강제한다. 어떤 서버에 진행 중 체인이 이미
2건 이상 있는 상태에서 올리면 인덱스 생성이 실패하니(사실상 개발 중 만든 데이터에서만
발생), 그 경우 해당 `pool_chains` 행을 정리하고 다시 올린다.

**에이전트 재빌드·재설치.** 풀 판정의 새 입력(사용량 미측정 계정 제외, `relogin_required`
격리, `usage_fetched_at` 신선도, `deliver_lock_timeout` 표식)은 **에이전트가 새 코드일 때만**
중앙에 온다. 서버를 자동 모드로 켜기 전에 각 에이전트를 최신으로 올린다. 절차는 §5-2
원클릭 재설치 또는 self-update(§DEV-TEST-GUIDE §4).

**운영 절차.** 콘솔 "계정 풀" 탭에서 서버별 정책을 **수동**으로 두고 며칠 관측한다. 배급처·
대여중·충전소 분포와 교체 권고가 상식과 맞는지 확인한 뒤에 그 서버만 **자동**으로 올린다.
테넌트 전체 자동화는 탭 상단 "자동화 일시정지"로 한 번에 멈춘다(이미 도는 체인은 끝까지
간다).

**수동 배정이 409로 거부될 때.** 어떤 서버에서 풀 교체 체인이 도는 동안 그 서버의 배정에
전달·회수·즉시전환을 걸면 `409 pool.chain_active`로 막힌다. 체인이 끝나길 기다리거나 탭에서
그 체인을 확인한다. 지금 당장 떼어내야 하면 예외는 하나, global-admin의 **force recall**
(`POST …:recall {"force": true}`)뿐이다. 이건 진행 중 체인을 무효로 실패 처리하고 회수를
밀어붙인다(§5.2 force recall과 같은 문).
