...
### 비대화 회수와 라이브 로그아웃 (claude, G46)
AMS가 활성 계정을 회수하면 에이전트는 `tsamx remove --yes`를 돌린다. 회수 대상이 그
호스트에서 로그인 중인 계정이면 백업뿐 아니라 라이브 자격(`<config home>/.credentials.json`과
`.claude.json`의 oauthAccount)까지 지운다. 회수된 계정이 호스트에 남아 사람이 직접
`claude`를 띄웠을 때 다시 과금되는 일을 막기 위해서다. 그래서 활성 계정을 회수한 직후
그 호스트는 로그아웃 상태가 되고, 다음 배정이나 `tsamx switch`로 다른 계정을 올리기
전까지는 자격이 없다. 소유 판정은 계정 uuid로 하므로 같은 이메일을 다른 조직에서 쓰는
계정이 로그인 중이면 건드리지 않는다. 대화형 `tsamx remove`(프롬프트)와 TUI·메뉴바
회수는 종전대로 백업만 지우고 라이브 로그인은 그대로 둔다.
...
한 가지 한계가 있다. tsamx가 세션마다 만드는 임시 세션 프로필에는 이 부트스트랩이
손대지 않으므로, 그 프로필로 뜬 세션에는 Langfuse Stop 훅이 걸려 있지 않다. 세션
프로필 경로로 직접 기동한 세션은 추적에서 빠질 수 있다. 러너 홈(`~/.claude-amx`)을
그대로 쓰는 `amx` 경유 세션은 §8의 훅 설치가 적용되므로 이 공백에 해당하지 않는다.
