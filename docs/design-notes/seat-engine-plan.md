...
- **OAuth 토큰 갱신 이식(선결).** P4 수집기는 액세스 토큰을 받기만 하고 갱신을
  하지 않는다. Claude Code가 돌지 않는 유휴 프로파일은 토큰이 만료된 채로 남고,
  그러면 그 계정의 사용량을 영영 읽지 못해 `token_expired` → 미측정 → 전환 후보
  제외로 조용히 굳는다. tsamx `oauth.py`의 refresh 흐름(`try_refresh_oauth_
  credentials`)을 이식해야 활성 계정 밖의 후보를 측정할 수 있다. 갱신 실패가
  `invalid_grant`로 누적될 때만 격리(`relogin_required`)로 승격한다 — 단순 만료를
  격리로 올리면 건전한 계정이 전부 격리된다.
- **사용량 상태 저장소(선결).** P4의 플래너는 순수 함수라 이전 관측·이전 간격·
  최근 429·백오프 종료 시각을 호출자가 들고 있어야 한다. tsamx는 이를
  `usage_store.py`(캐시 파일 + 서브 TTL 게이트 + 프로세스 간 claim lease)로
  관리한다. 같은 계정을 여러 프로세스가 보는 상황에서 리스가 없으면 예산을
  중복 소비한다. 저장소와 리스를 P5에서 만든다.
...
- **섀도 기간에 새 수집기는 API를 직접 치지 않는다.** 폴링 예산은 계정(identity)
  단위이고 tsamx가 이미 그 예산을 쓰고 있어, 두 수집기가 동시에 발사하면 같은
  예산을 이중 소비해 429를 자초한다. 그 429는 운영 경로인 tsamx까지 막는다.
  따라서 섀도 단계에서는 새 엔진이 tsamx의 사용량 캐시를 읽어 판정만 비교하고,
  직접 폴링은 기본값이 네이티브로 넘어간 뒤에 켠다. 429는 요청자가 아니라
  계정에 걸리므로 User-Agent를 갈라도 유발자를 사후에 가릴 수 없다 — 귀속은
  양쪽 로컬 요청 로그(시각·엔진·계정)로 한다.
...
1. **로테이션 범위 정책 = `owner` | `server`, 기본값은 `server`(08-23 리뷰 F1로
   수정).** 소유자 라벨이 빈 계정은 조직 공용으로 해석해 전 서버에 배정
   가능하고, 라벨이 있는 계정은 `owner` 범위에서만 같은 소유자의 서버로
   좁혀진다. 결정 자체(축의 존재, 판정 규칙)는 유지하되 **기본값 전환은 별도
   단계다**: 실측으로 이미 owner 라벨이 붙은 계정이 존재하는데(dev DB
   `modarra9@gmail.com`) 서버 owner는 전부 비어 있어, `owner`를 기본값으로
   두면 그 계정이 라벨 없는 서버 전체에서 조용히 후보를 잃는다(08-22 실명
   스왑 사건과 같은 형태). 그래서 `server`(현행과 동일)를 기본값으로 두고,
   운영자가 서버·계정 owner 라벨을 정비한 뒤 서버별로 `owner`를 옵트인하는
   것을 다음 단계로 남긴다. 계정 폼에는 이미 소유자 입력이 있고(RegisterModal·
