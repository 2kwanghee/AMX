...
3. **전달** — 할당 행의 `전달` 버튼 → 상태가 `전달 중` → 몇 초 안에 **`활성`**(에이전트가 수신·적용 확인).
   - **성공 판정(노트북)**: `tsclaude list` 에 그 계정이 보임(설치 시 각인된 설정 홈 조회 래퍼; `CLAUDE_CONFIG_DIR=~/.claude-amx tsamx list`와 동일).
   - **성공 판정(화면)**: 할당 행이 "이메일 → 서버명" 파이프라인으로 `활성` 표시.
4. **사용 확인(선택)** — 노트북에서 `CLAUDE_CONFIG_DIR=~/.claude-amx claude` 실행 → 로그인 요구 없이 동작하면 끝까지 통한 것.
5. **회수** — 할당 행의 `회수` → 에이전트가 계정을 그 서버에서 완전히 뺌(자격증명·매니페스트 삭제).
   - **성공 판정**: `tsclaude list` 에서 사라지고 화면 상태가 `해제됨`(detached). 이력 행은 감사용으로 남고, 재사용은 재전달.
