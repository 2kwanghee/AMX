...
| G46 | **단일계정 활성 회수 시 라이브 `~/.claude` 자격 잔존** — **08-16 실측 재현**: 다계정 풀(3개)에서도 활성 khee recall 시 switch-away가 일어나지 않아, purge 후 풀에는 없는데 라이브 자격은 khee로 남았다(고아 자격). 재배정으로 정합 복원됨. adversary 예상(다계정이면 switch-away)과 달리 다계정에서도 잔존 — — `tsamx remove`가 활성 계정을 지울 때 다른 계정이 없으면 스위치가 없고(handlers.go otherSwitchTarget 빈 문자열), `_delete_account_files`는 백업 사본만 지우고 라이브 로그인(`~/.claude.json`·`.credentials.json`)은 남긴다. 회수 후 그 호스트에서 사람이 직접 `claude`를 실행하면 회수된 계정이 그대로 재사용될 수 있다 → 오과금·격리 표면. `--yes` 도입으로 이 단일계정 활성 회수 경로에 비로소 도달 가능해졌다. remove 시 활성이면 라이브 자격까지 무효화(logout)할지 결정 필요 | G45 수정 중 발견(08-15) |
