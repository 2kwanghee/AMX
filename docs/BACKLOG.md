...
| G48 | **러너 tsamx의 업데이트 경로 부재** — tsamx는 `uv tool install`로 설치되고 에이전트 SelfUpdate는 ama 바이너리만 갱신한다. 08-16 라이브 왕복 테스트에서 8/14 설치본 tsamx가 `--yes`(#85)를 몰라 recall이 `tsamx_remove`로 DIVERGED — `uv tool install --force --from <체크아웃>/tsamx tsamx` 재설치로 해소. SelfUpdate 또는 agent-setup에 tsamx 갱신 단계 필요, fleet 배포 시에도 동일 | 라이브 왕복 테스트(08-16) |
