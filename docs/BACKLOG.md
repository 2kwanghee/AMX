...
| G45 | **e2e recall이 수렴 실패로 깨짐 — 원인은 단언 고착이 아니라 `tsamx remove` 비대화 경로 부재(생산 버그)** — 08-15 실행 확인(`test_p2_channel_e2e.py`): 실패는 재현되나 위치가 가설(205/208)이 아니라 **line 194**다. 회수 할당이 `detached`에 못 가고 `recalling`에 갇힌다. 근본 원인: 08-14 O2 변경으로 AMS가 항상 purge=true를 보내 에이전트가 `tsamx remove` 경로를 타는데(handlers.go:341), `tsamx remove`는 `input("...[y/N]")` 대화형 확인을 요구하고(switcher.py:2700-2704, cli.py:1236이 assume_yes 없이 호출·remove엔 `--yes`/`--force` 없음) 에이전트 exec 브리지는 stdin을 연결하지 않아(exec.go:130-134) **EOFError로 63ms 만에 즉시 실패** → recall DIVERGED(host-a applied.log 확인, 스위치 후 +119ms). 단위 테스트는 Fake 브리지라 못 잡음. 단언 205/208도 purge 의미론상 stale이나, 1차 차단은 이 수렴 버그이므로 **단언 정정만으로는 통과 불가**. 통과에는 생산 코드 수정 필요(`tsamx remove --yes`/assume_yes 또는 브리지가 확인 우회) — 이 작업 범위(생산 코드 금지) 밖, 별도 트랙 결정 필요 | 문서 감사(08-15) · 실행 확인(08-15) |
