...
| G51 | **활성 슬롯 판정의 잔여 복합키 지점 6곳** — G49 수정은 게이트 경로(list/current_account_number)만 uuid-first로 전환. status(switcher.py:4190, 발산 시 managed:false 오표기), autoswitch 기준점(4494), add-in-place(2324), self-switch 가드(4811), resync(4943), _perform_switch(5240)은 여전히 (email,org) 복합키라 org 발산 시 miss — 코드베이스가 "라이브 슬롯" 질문에 두 방식으로 답한다. _resolve_active_slot로 통일 검토 | G49 리뷰(08-16) |
