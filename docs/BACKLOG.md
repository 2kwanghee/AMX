...
| G50 | **usage 스냅샷 수신 시각 스탬핑의 귀속 왜곡** — `reported_at=server_default now()`(models.py:502)이고 ingest가 override하지 않아(grpc/server.py:728), 장기 오프라인 에이전트의 outbox 재전송분이 원발생일이 아니라 복귀일에 귀속된다. 정산 무결성은 유지되나(모두 청구됨) 일자 귀속이 왜곡됨. event-time 전환 시 G26 지각 도착 경로가 실제로 열리므로 두 건은 함께 설계해야 함 | G26 재검토에서 발견(08-16) |
