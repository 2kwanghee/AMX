...
| ~~G48~~ 해소 | **러너 tsamx의 업데이트 경로 부재** — ~~tsamx는 `uv tool install`로 설치되고 에이전트 SelfUpdate는 ama 바이너리만 갱신한다. 08-16 라이브 왕복 테스트에서 8/14 설치본 tsamx가 `--yes`(#85)를 몰라 recall이 `tsamx_remove`로 DIVERGED~~ **해소:** ① `agent-setup.sh` install이 `uv tool install --force --from "$ROOT/tsamx" tsamx`로 체크아웃 기준 재설치(이미 있어도 갱신, 멱등), ② ama-agent self_update가 바이너리 스왑·applied 로그·ack 뒤 exec **직전** 같은 명령으로 tsamx 재설치(git 설치 방식 한정, 실패는 경고 로그만 — self_update CONVERGED 불변, 다음 주기 재시도), ③ PROD-GUIDE·DEPLOYMENT-RUNNER에 "tsamx는 설치·자기갱신과 함께 갱신" 명시. fleet 재배포도 별도 tsamx 절차 불필요 | 라이브 왕복 테스트(08-16) · 수정(08-16) |
