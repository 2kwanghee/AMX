...
| G49 | **AMA switch-away가 AMS org-uuid 미스테이징으로 영구 미발동** — G46 조사에서 확정. 회수 시 활성 계정을 다른 계정으로 넘기는 handlers.go:328 스위치는 `tsamx list`의 활성 판정에 걸려 있고 그 판정은 (email, organizationUuid) 복합키인데, claude.go StageCredential이 organizationUuid를 채우지 않아(claude.go:95-97) 라이브 config의 org와 슬롯이 어긋나면 활성 판정이 빈 문자열을 돌려주고 스위치가 끝내 걸리지 않는다. G46 수정으로 라이브 자격 잔존은 tsamx 층에서 막았지만, 회수 뒤 다른 활성 계정으로 넘어가는 전환 자체는 여전히 일어나지 않아 그 호스트는 재배정·`tsamx switch` 전까지 무자격으로 남는다. 근본 해소는 스테이징에 org uuid를 싣거나 활성 판정을 uuid-first로 바꾸는 것 — 별도 수정 후보 | G46 조사에서 발견(08-16) |
