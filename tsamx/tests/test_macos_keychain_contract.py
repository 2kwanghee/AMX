...
1. **Mocked tests** (run on every PR, every platform): assert that the macOS
   ...
   `macos_keychain` security wrapper, under the new `tsamx` service. This
...
class TestBackupCredentialsSecurity:
    ...
    per-account backup service is the new ``tsamx`` (not the old keyring
    ``claude-code``).
    """
    ...
    def test_read_account_credentials_uses_security_service(
        ...
        with patch("tsamx.credentials.macos_keychain") as mock_kc:
            ...
            mock_kc.get_password.assert_called_once_with(
                "tsamx", "account-1-user@example.com"
            )
    ...
    def test_write_account_credentials_uses_security_service(
        ...
        with patch("tsamx.credentials.macos_keychain") as mock_kc:
            ...
            mock_kc.set_password.assert_called_once_with(
                "tsamx", "account-2-alice@example.com", "secret-token"
            )
    ...
    def test_write_retains_prev_generation_in_keychain_not_a_file(
        ...
        with patch("tsamx.credentials.macos_keychain") as mock_kc:
            ...
            mock_kc.set_password.assert_has_calls([
                call("tsamx", "account-2-alice@example.com.prev",
                     "old-generation"),
                call("tsamx", "account-2-alice@example.com",
                     "secret-token"),
            ])
    ...
    def test_delete_account_credentials_uses_security_service(
        ...
        with patch("tsamx.credentials.macos_keychain") as mock_kc:
            ...
            mock_kc.delete_password.assert_has_calls([
                call("tsamx", "account-3-bob@example.com"),
                call("tsamx", "account-3-bob@example.com.prev"),
                call("tsamx", "account-None-bob@example.com"),
                call("tsamx", "account-None-bob@example.com.prev"),
            ])
...
def test_wrapper_roundtrip_real_keychain(tmp_keychain: str):
    ...
    """
    macos_keychain.set_password("tsamx-test", "acct-1", "round-trip-token")
    assert macos_keychain.get_password("tsamx-test", "acct-1") == "round-trip-token"
    macos_keychain.delete_password("tsamx-test", "acct-1")
    assert macos_keychain.get_password("tsamx-test", "acct-1") is None
