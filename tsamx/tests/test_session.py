"""Tests for session mode (tsamx.session + the switcher guards)."""
...
from tsamx import session as session_mod
from tsamx.credentials import CLAUDE_CODE_MANAGED_KEYCHAIN_SERVICE
...
from tsamx.paths import get_global_config_path
...
class TestRun:
    ...
    def test_auth_override_vars_scrubbed_from_session_env(
        ...
        # `tsamx run 2` means account 2, not whatever the API key resolves to.
...
class TestGuards:
    ...
    def test_backup_credential_write_invalidates_stale_profile(
        ...
        recovery path leaves `tsamx run` on stale credentials that still pass
    ...
    def test_list_skips_refresh_for_live_session_accounts(
        ...
        """tsamx --list must not proactively refresh an account that is live in
        ...
        monkeypatch.setattr(
            "tsamx.oauth.try_fetch_usage_for_account", fake_fetch
        )
    ...
    def test_unreadable_keychain_fails_closed(
        ...
        def locked(service: str, account: str) -> str | None:
            if not service.startswith("Claude Code-credentials-"):
                return fake_store_read(service, account)  # tsamx's own backup store
