"""Tests for the `tsamx config` subcommand (get/set/unset/list/path)."""
...
class TestConfigMisc:
    ...
    def test_main_help_mentions_config(self, temp_home, capsys):
        with patch.object(sys, "argv", ["tsamx", "--help"]):
    ...
    def test_auto_picks_up_configured_threshold(self, temp_home, capsys):
        """End-to-end: a value set via config drives `tsamx auto`."""
        ...
        with patch("tsamx.autoswitch.AutoSwitchEngine", FakeEngine), \
