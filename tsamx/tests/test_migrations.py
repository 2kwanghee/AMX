"""Tests for one-time data migrations (tsamx.migrations).
...
keyring`` locally, patching ``tsamx.switcher.keyring`` would NOT affect
...
from tsamx.switcher import KEYRING_SERVICE, ClaudeAccountSwitcher
...
class TestWindowsFileBackend:
    def test_round_trip_uses_files_not_keyring(self, temp_home):
        ...
        with patch("tsamx.switcher.keyring", create=True) as mock_keyring:
...
class TestMacosKeyringToSecurity:
    ...
    service (``tsamx``). The autouse guard fakes the security backend
