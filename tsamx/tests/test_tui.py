...
from tsamx.autoswitch import NoSwitchEvent, SwitchEvent
from tsamx.json_output import USAGE_API_KEY, USAGE_TOKEN_EXPIRED
from tsamx.models import AccountSnapshot, AccountsSnapshot
from tsamx.switcher import ClaudeAccountSwitcher
from tsamx.tui import data as tui_data
from tsamx.usage_store import UsageEntry
...
async def menu_select(pilot, action_id: str) -> None:
    ...
    from tsamx.tui.widgets import MenuItem
...
class TestFormatting:
    ...
    def test_sentinel_labels_match_tsamx_list(self):
        # The TUI must describe sentinel states with the exact wording `tsamx
        ...
        from tsamx.switcher import SENTINEL_NOTES
    ...
    def test_sentinel_card_shows_last_seen_like_tsamx_list(self):
        # A sentinel is a live overlay — the entry can still carry the last
        # good measurement, and `tsamx list` prints it as a "last seen" line.
        # The card must too (except for API-key accounts, which have no quota).
        from tsamx.tui.widgets import account_card_text
    ...
    def test_account_card_uses_light_palette_when_passed(self):
        from tsamx.tui.theme import ACCENT_LIGHT, TSAMX_LIGHT, Palette
        from tsamx.tui.widgets import account_card_text

        acc = make_account(1, active=True, entry=make_entry(pct5=95.0))
        text = account_card_text(acc, 100, palette=Palette.from_theme(TSAMX_LIGHT))
...
class TestSnapshotSource:
    ...
    def test_every_pass_is_store_governed(self, tmp_path):
        ...
        # reservation), so every take is the same on-demand pass `tsamx list`
...
class TestUsageRows:
    ...
    def test_absent_window_produces_no_row(self):
        from tsamx.tui.widgets import usage_rows
...
class TestMiniAccountText:
    def test_seven_day_ahead_of_pace_marker(self):
        from tsamx.tui.widgets import mini_account_text
...
class TestDashboard:
    async def test_panel_shows_active_full_and_others_mini(self, tmp_path):
        ...
        async with app.run_test(size=(100, 32)) as pilot:
            await settle(pilot)
            from tsamx.tui.widgets import AccountsPanel
    ...
    async def test_s_opens_switch_screen_and_enter_switches(self, tmp_path):
        ...
        async with app.run_test(size=(100, 32)) as pilot:
            ...
            from tsamx.tui.dashboard import DashboardScreen, SwitchScreen
            from tsamx.tui.widgets import AccountItem
    ...
    async def test_remove_via_menu_confirms_then_removes(self, tmp_path):
        ...
        async with app.run_test(size=(100, 32)) as pilot:
            ...
            from tsamx.tui.modals import ConfirmModal
    ...
    async def test_palette_is_disabled(self, tmp_path):
        ...
        assert TsamxApp.ENABLE_COMMAND_PALETTE is False
...
class TestWatchScreen:
    ...
    async def test_w_opens_monitor_without_cursor(self, tmp_path):
        ...
        async with app.run_test(size=(100, 40)) as pilot:
            ...
            from tsamx.tui.dashboard import WatchScreen
            from tsamx.tui.widgets import AccountItem
    ...
    async def test_escape_disarms_then_leaves(self, tmp_path):
        ...
        async with app.run_test(size=(100, 40)) as pilot:
            ...
            from tsamx.tui.dashboard import DashboardScreen, WatchScreen
    ...
    async def test_app_start_watch_stacks_over_dashboard(self, tmp_path):
        ...
        app = TsamxApp(self._fake(tmp_path), start="watch")
        async with app.run_test(size=(100, 40)) as pilot:
            await settle(pilot)
            from tsamx.tui.dashboard import DashboardScreen, WatchScreen
...
def fake_engine(monkeypatch):
    ...
    monkeypatch.setattr(
        "tsamx.tui.autoview.AutoSwitchEngine", _FakeEngine
    )
    return _FakeEngine
...
class TestAutoScreen:
    ...
    async def test_opens_in_dry_run_and_store_only(self, tmp_path, fake_engine):
        ...
        async with app.run_test(size=(100, 40)) as pilot:
            ...
            from tsamx.tui.autoview import AutoScreen
    ...
    async def test_back_stops_engine_and_restores_fetching(
        ...
        async with app.run_test(size=(100, 40)) as pilot:
            ...
            await pilot.press("escape")
            await settle(pilot)
            from tsamx.tui.dashboard import DashboardScreen
...
class TestEventText:
    def test_switch_event_styling_and_content(self):
        ...
        from tsamx.tui.autoview import event_text
    ...
    def test_event_text_uses_light_accent_for_switch(self):
        ...
        text = event_text(event, palette=Palette.from_theme(TSAMX_LIGHT))
...
class TestBareInvocation:
    def test_bare_tty_launches_tui(self, monkeypatch, temp_home):
        import tsamx.cli as cli
        import tsamx.tui as tui
        ...
        monkeypatch.setattr(sys, "argv", ["tsamx"])
    ...
    def test_tsamx_watch_opens_tui_on_watch_page(self, monkeypatch, temp_home):
        import tsamx.cli as cli
        import tsamx.tui as tui
        ...
        monkeypatch.setattr(sys, "argv", ["tsamx", "watch"])
        monkeypatch.setattr(tui, "run", fake_run)
        with pytest.raises(SystemExit) as excinfo:
            cli.main()
        assert excinfo.value.code == 0
...
class TestThemeWiring:
    ...
    async def test_auto_setting_no_detection_falls_back_to_dark(self, tmp_path):
        ...
        fake = FakeSwitcher([make_account("1", active=True)], tmp_path)
        from tsamx.tui.app import TsamxApp
        app = TsamxApp(fake, detected=None)
        async with app.run_test() as pilot:
            await settle(pilot)
            assert app.theme == "tsamx-dark"
    ...
    async def test_toggle_cycles_dark_light_auto(self, tmp_path):
        ...
        async with app.run_test() as pilot:
            await settle(pilot)
            assert app.theme == "tsamx-dark"          # setting dark
            app.action_toggle_theme(); await pilot.pause()
            assert app.theme == "tsamx-light"          # → light
            app.action_toggle_theme(); await pilot.pause()
            assert app.theme == "tsamx-light"          # → auto, detected=light
            assert json.loads((tmp_path / "settings.json").read_text())["ui"]["theme"] == "auto"
            app.action_toggle_theme(); await pilot.pause()
            assert app.theme == "tsamx-dark"           # → back to dark
