...
from tsamx import macos_keychain as _macos_keychain
...
def _isolate_real_home(request, tmp_path_factory, monkeypatch):
    ...
    that switcher resolves to the real ``~/.tsamx-backup`` — writing logs,
...
def block_real_keychain(request, monkeypatch):
    ...
    Replaces the ``security``-CLI wrapper (``tsamx.macos_keychain``) with an
...
def _deterministic_poll_jitter(monkeypatch):
    ...
    monkeypatch.setattr("tsamx.poll_policy.JITTER_FRAC", 0.0)
...
def _deterministic_colour(monkeypatch):
    ...
    from tsamx import appearance as _appearance
    from tsamx import printer as _printer
    ...
    monkeypatch.setattr("tsamx.printer._colors_enabled", None)
    ...
    # FORCE_COLOR.
    monkeypatch.setattr("tsamx.printer._theme", "dark")
    ...
    monkeypatch.setattr("tsamx.appearance._cache", _appearance._UNSET)
