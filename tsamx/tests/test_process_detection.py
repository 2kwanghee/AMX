...
from tsamx.process_detection import (
    ClaudeSession,
    IdeInstance,
    get_claude_dir,
...
from tsamx.printer import abbreviate_path, entrypoint_label, format_age
...
class TestIsPidAlive:
    ...
    def test_alive_pid(self):
        with patch("tsamx.process_detection.sys.platform", "linux"), \
    ...
    def test_windows_dispatches_to_ctypes_impl(self):
        ...
        with patch("tsamx.process_detection.sys.platform", "win32"), \
             patch(
                 "tsamx.process_detection._is_pid_alive_windows",
                 return_value=True,
             ) as mock_win:
            assert is_pid_alive(12345) is True
...
class TestListSessions:
    def test_reads_valid_sessions(self, tmp_path):
        ...
        with patch("tsamx.process_detection.is_pid_alive", return_value=True):
    ...
    def test_filters_dead_pids(self, tmp_path):
        ...
        with patch("tsamx.process_detection.is_pid_alive", side_effect=alive):
            result = list_sessions(tmp_path)
...
class TestListIdeInstances:
    ...
    def test_filters_dead_pids(self, tmp_path):
        ...
        with patch("tsamx.process_detection.is_pid_alive", side_effect=lambda p: p == 2001):
            result = list_ide_instances(tmp_path)
