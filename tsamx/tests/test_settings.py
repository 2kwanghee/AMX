...
class TestAtomicWriteThroughSymlink:
    ...
    stays on the directory tsamx owns (else it narrows — or cannot touch —
    a foreign one)."""
    ...
    def test_temp_file_is_created_beside_the_target(self, tmp_path, monkeypatch):
        ...
        import tempfile
        from tsamx import settings as S
        repo = tmp_path / "repo"; repo.mkdir()
        live = tmp_path / "live"; live.mkdir()
    ...
    def test_hardening_stays_on_the_directory_tsamx_owns(self, tmp_path):
        """The 0700 belongs to tsamx's own dir. On the target's parent it
