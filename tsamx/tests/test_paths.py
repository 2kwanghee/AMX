"""Tests for tsamx.paths resolver helpers.

These tests verify that tsamx resolves Claude Code config/credential paths the
same way claude-code itself does. If these drift from claude-code's behavior,
tsamx will read the wrong files and misattribute accounts (see issue #16).
"""
...
from tsamx.exceptions import MigrationError
...
class TestGetBackupRoot:
    """Linux/WSL: XDG-aware path. Other platforms: legacy ~/.tsamx-backup."""
    ...
    def test_linux_default_is_xdg_data_home(
        ...
        assert get_backup_root() == isolated_home / ".local" / "share" / "tsamx"
    ...
    def test_linux_respects_xdg_data_home(
        ...
        monkeypatch.setattr(Platform, "detect", staticmethod(lambda: Platform.LINUX))
        assert get_backup_root() == custom / "tsamx"
    ...
    def test_linux_expands_tilde_in_xdg_data_home(
        ...
        monkeypatch.setattr(Platform, "detect", staticmethod(lambda: Platform.LINUX))
        assert get_backup_root() == isolated_home / "custom-data" / "tsamx"
    ...
    def test_legacy_helper_returns_home_dot_tsamx_backup(
...
class TestMigrateLegacyBackupDir:
    def test_no_legacy_is_noop(self, isolated_home: Path):
        target = isolated_home / ".local" / "share" / "tsamx"
    ...
    def test_target_with_only_throwaway_artifacts_is_wiped(self, isolated_home: Path):
        ...
        any prior tsamx invocation laid down cache/ + tsamx.log in the
        ...
        (target / "tsamx.log").write_text("noise")
        (target / "tsamx.log.1").write_text("rotated")
        ...
        assert not (target / "tsamx.log").exists()
    ...
    def test_oserror_is_wrapped(self, isolated_home, monkeypatch):
        ...
        monkeypatch.setattr("tsamx.paths.shutil.move", exploding_move)
