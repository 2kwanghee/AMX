...
only — the auto-switch engine itself lives in ``tsamx.autoswitch`` and is
tested there.
"""
...
from tsamx import menubar
from tsamx.exceptions import ClaudeSwitchError
from tsamx.switcher import USAGE_API_KEY
...
def test_notification_identity_creates_and_preserves_info_plist(tmp_path: Path):
    ...
    assert data["CFBundleIdentifier"] == "com.tsamx.menubar"
    assert data["CFBundleName"] == "tsamx"
...
def test_run_without_rumps_raises_clean_error(monkeypatch):
    ...
    around ``from tsamx.menubar import run`` can never fire — the import
    ...
    with pytest.raises(ClaudeSwitchError, match=r"tsamx\[menubar\]"):
