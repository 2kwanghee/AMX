...
from tsamx.tui import theme
from tsamx.tui.theme import TSAMX_DARK, TSAMX_LIGHT, Palette
...
def test_from_theme_reads_theme_object_including_track():
    p = Palette.from_theme(TSAMX_LIGHT)
...
def test_both_themes_expose_track_variable():
    assert TSAMX_DARK.variables["track"] == theme.TRACK
    assert TSAMX_LIGHT.variables["track"] == theme.TRACK_LIGHT
    assert TSAMX_LIGHT.dark is False
