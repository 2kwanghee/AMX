...
from tsamx.exceptions import TransferError
from tsamx.models import Platform
from tsamx.switcher import ClaudeAccountSwitcher
from tsamx.transfer import export_accounts, import_accounts
...
class TestRoundTrip:
    def test_export_import_round_trip(self, temp_home: Path):
        ...
        out_file = temp_home / "backup.tsamx"
...
class TestConflictPolicy:
    ...
    def test_import_over_live_login_hints_force_activation(
        ...
        assert "tsamx --switch-to 1 --force" in err
...
class TestValidation:
    ...
    def test_missing_file_rejected(self, temp_home: Path):
        ...
        with pytest.raises(TransferError, match="not found"):
            import_accounts(s, str(temp_home / "nope.tsamx"))
...
class TestExportSkipsBrokenSlots:
    ...
    def test_explicit_account_with_missing_credentials_hard_fails(
        ...
        from tsamx.exceptions import CredentialReadError

        with pytest.raises(CredentialReadError, match="no backup credentials"):
            export_accounts(
                s, str(temp_home / "x.tsamx"), account="1"
            )
    ...
    def test_stdout_pipe_mode_keeps_stdout_pure_json(
        ...
        """tsamx --export - must produce valid JSON on stdout even when one
...
class TestImportSessionInvalidation:
    ...
    def test_force_overwrite_invalidates_session_credentials(
        ...
        # Credential material dropped → next `tsamx run` re-bootstraps from
...
class TestImportClearsDeadTokenQuarantine:
    ...
    def test_reimport_after_removal_lifts_orphan_quarantine(
        ...
        """The case the overwrite-only fix would miss: `tsamx remove` never
        ...
        # Simulate `tsamx remove 2`: drop the sequence entry, leave the
    ...
    def test_plain_import_heal_warns_about_live_session(
        ...
        keeps its own credential copy until restarted via `tsamx run`."""
        import os as _os
