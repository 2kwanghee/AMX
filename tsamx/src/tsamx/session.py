...
``tsamx run NUM|EMAIL`` launches Claude Code with ``CLAUDE_CONFIG_DIR``
...
A manifest records what tsamx created so removal never touches user data.
...
from tsamx.claude_locks import proper_lockfile
from tsamx.exceptions import (
    ClaudeCodeLockTimeout,
    CredentialReadError,
    SessionError,
)
from tsamx.fsutil import replace_with_retry
from tsamx.locking import FileLock
from tsamx.models import Platform
from tsamx.oauth import refresh_oauth_credentials
from tsamx.paths import get_default_global_config_path
from tsamx.printer import accent, dimmed, muted, warning
from tsamx.process_detection import ClaudeSession, list_sessions
from tsamx.settings import atomic_write_json
...
# Records which entries in a session profile tsamx created (so --no-share and
# re-syncs only ever remove tsamx-managed links/copies, never user data).
SHARE_MANIFEST = ".tsamx-shared.json"
...
# profile must be re-bootstrapped on the next non-live `tsamx run` even if it
# still passes the local reuse check.
STALE_MARKER = ".tsamx-stale-credentials"

# The user-scope MCP key mirrored from the default profile's .claude.json.
MCP_KEY = "mcpServers"

# Adoption marker: this profile's mcpServers is (or was) tsamx-mirrored. Gates
...
MCP_MIRROR_MARKER = ".tsamx-mcp-mirror-v1"
...
MCP_DISPLACED_STASH = ".tsamx-mcp-displaced.json"
...
# warning: `tsamx run N` is an explicit request for account N, so letting an
...
def read_session_credentials(session_dir: Path) -> str | None:
    ...
    location stays claude's job (see the module docstring on why tsamx never
...
class SessionManager:
    ...
    def exec_default(self, claude_args: list[str]) -> NoReturn:
        ...
        Used by `tsamx run` (no account) when the cwd has no mapping, or its
    ...
    def _exec(self, claude_bin: str, claude_args: list[str], env: dict[str, str]) -> NoReturn:
        ...
        POSIX: ``execvpe`` replaces the tsamx process entirely (the lock is
    ...
    def _ensure_not_api_key(self, account_num: str, email: str) -> None:
        ...
        if self.switcher._account_kind(account_num) == "api_key":
            raise SessionError(
                ...
                "'tsamx run' (session mode) does not support API-key accounts yet. "
                "Use 'tsamx --switch-to' to make it your default login instead."
            )
    ...
    def setup_session(
        ...
        # a second `tsamx run` joining a live session must not invalidate
        ...
        with FileLock(self.switcher.lock_file, timeout=_BOOTSTRAP_LOCK_TIMEOUT):
            ...
            # another `tsamx run` may have bootstrapped while we waited.
            ...
            if not self._is_session_valid(session_dir, email, org_uuid):
                ...
                raise SessionError(
                    ...
                    f"tsamx --add-account --slot {account_num}"
                )
    ...
    def _bootstrap(
        ...
        if not creds:
            raise SessionError(
                ...
                f"Re-add with: tsamx --add-account --slot {account_num}"
            )
        ...
        for name in active_items:
            ...
            if dest.is_symlink():
                if name not in managed:
                    managed = [*managed, name]  # adopt: only tsamx links here
                if use_symlinks:
                    try:
    ...
    def _sync_mcp_servers(self, session_dir: Path, share: bool) -> None:
        ...
        a session are overwritten the next time tsamx prepares the profile.
    ...
    def _read_mcp_source() -> dict | None:
        ...
        (ignoring CLAUDE_CONFIG_DIR — a nested `tsamx run` must not source
    ...
    def _write_manifest(self, manifest_path: Path, items: list[str]) -> None:
        ...
        fd, tmp = tempfile.mkstemp(
            dir=str(manifest_path.parent), prefix=".tsamx-shared-", suffix=".tmp"
        )
        try:
    ...
    def _remove_managed(dest: Path) -> None:
        """Remove a tsamx-created share entry (link or copy), never user data
