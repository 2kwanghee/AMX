"""Export and import account data for tsamx.
...
(e.g. `tsamx --export - | gpg -c > out.gpg`).
"""
...
from tsamx.credentials import looks_like_api_key
...
from tsamx.models import Platform, get_timestamp, normalize_alias
...
def export_accounts(
    ...
    if not sequence_data or not sequence_data.get("accounts"):
        raise TransferError("no accounts to export — run tsamx --add-account first")
    ...
    if not accounts_payload:
        raise TransferError(
            ...
            "credentials/config. Re-add with: tsamx --add-account --slot <number>"
        )
...
def import_accounts(
    ...
    if envelope.get("encrypted") is True:
        raise TransferError(
            ...
            "decrypt before piping (e.g. gpg -d backup.gpg | tsamx --import -)"
        )
    ...
    for entry in normalized:
        ...
        if existing_slot is not None:
            ...
            # the next `tsamx run` re-bootstraps from the imported creds. A
    ...
    # "I forced this" from "tsamx healed this". Appended only when it
    ...
    if identity is not None and final is not None:
        ...
        if live_slot is not None and live_slot in written_slots:
            _eprint(
                ...
                f"imported credentials with: tsamx --switch-to {live_slot} --force"
            )
