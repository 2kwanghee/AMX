...
(so cron-driven ``tsamx auto --once`` ticks behave across processes), mutated
...
from tsamx.json_output import SCHEMA_VERSION, USAGE_TOKEN_EXPIRED
from tsamx.locking import FileLock
from tsamx.poll_policy import (
    ESCALATION_MARGIN_PCT,
    RESET_SLACK_S,
    binding_pct,
)
from tsamx.settings import AutoSwitchSettings, atomic_write_json, parse_model_names
from tsamx.switcher import ClaudeAccountSwitcher
from tsamx.usage_store import due_candidate, plan_oversleeps_interval

STATE_FILENAME = "autoswitch_state.json"
STATE_SCHEMA_VERSION = 1

_logger = logging.getLogger("tsamx")
...
class QuarantineEvent(AutoSwitchEvent):
    ...
    def human(self) -> str:
        return (
            ...
            f"Log in with it and run 'tsamx --add-account --slot {self.number}' "
            "to recover."
        )
...
def _window_pcts(
    ...
    correctly ignored it. Full per-model usage lives in ``tsamx list``.
    """
    return {
...
class AutoSwitchEngine:
    ...
    def _freshen_target(self, number: str, email: str) -> str:
        ...
        if self.switcher.live_session_pids_for(number, email):
            # A live `tsamx run` session owns this account's token in its own
    ...
    def _tick_inner(self) -> TickOutcome:
        ...
        if current is None:
            ...
            if self.switcher.has_live_login():
                # Live login exists but tsamx doesn't manage it: never act —
                # a switch would overwrite it without a backup.
                self._emit(
                    NoSwitchEvent(
                        reason="unmanaged-active-account",
                        detail="run 'tsamx --add-account' to include it in rotation",
                    )
                )
            else:
                self._emit(
                    NoSwitchEvent(
                        ...
                        detail="log in and run 'tsamx --add-account' first",
                    )
                )
    ...
    def _perform(
        ...
        # switch path (tsamx FileLock + Claude Code locks) never takes the
        # state lock.
        with self._state_lock():
            state = self._read_state()
            if trigger in ("proactive", "consume-first") and self._in_cooldown(state):
