"""macOS menu bar app for tsamx (``tsamx --menubar``).

A thin GUI shell over ``ClaudeAccountSwitcher`` and the core auto-switch engine
(``tsamx.autoswitch``) — it never re-implements account, usage, or
...
``AutoSwitchEngine`` the CLI's ``tsamx auto`` drives, sharing
...
from tsamx import pace
from tsamx.exceptions import ClaudeSwitchError, CredentialReadError
...
NOTIFICATION_BUNDLE_ID = "com.tsamx.menubar"
...
def ensure_notification_identity(
    ...
    *,
    ...
    try:
        ...
        logging.getLogger("tsamx").warning(
...
class MenuBarSettings:
    ...
    read/written through ``tsamx.settings`` (the ``autoswitch.*`` keys),
...
def run(switcher) -> int:
    """Entry point for ``tsamx --menubar``. Blocks until the user quits."""
    ...
    try:
        ...
        # guard around ``from tsamx.menubar import run`` can never see a
        ...
        raise ClaudeSwitchError(
            ...
            "Install with: pip install 'tsamx[menubar]'"
        ) from e
    ...
    from tsamx.autoswitch import AutoSwitchEngine
    from tsamx.settings import load_settings, set_setting
    from tsamx.snapshot_source import SnapshotSource

    settings_path = switcher.backup_dir / "menubar_settings.json"
    log_path = switcher.backup_dir / "tsamx.log"
    ...
    class MenuBarApp(rumps.App):
        def __init__(self):
            ...
            # Auto-switch engine (the same one `tsamx auto` runs), hosted in a
        ...
        def _start_engine(self):
            ...
            try:
                ...
                rumps.notification("tsamx", "Auto-switch failed to start", str(e))
                return
            self._engine = engine
        ...
        def _drain_engine_events(self):
            ...
            for ev in events:
                if ev.kind == "switch" and not getattr(ev, "dry_run", False):
                    rumps.notification("tsamx", "Auto-switched account", ev.human())
                    self.refresh_async()  # reflect the switch promptly
                elif ev.kind == "account-quarantined":
                    rumps.notification("tsamx", "Account quarantined", ev.human())
                elif ev.kind == "all-exhausted":
                    rumps.notification("tsamx", "All accounts exhausted", ev.human())
                    ...
                    rumps.notification("tsamx", "Configuration warning", ev.human())
        ...
        def _guard(self, fn):
            ...
            try:
                ...
                rumps.alert(title="tsamx", message=str(e))
                return False
            ...
            try:
                ...
                rumps.alert(
                    ...
                    message="Couldn't read the active credential. If the menu bar is running "
                            ...
                            "quit and relaunch it from a Terminal with: tsamx --menubar",
                )
                return
            except ClaudeSwitchError as e:
                rumps.alert(title="tsamx", message=str(e))
        ...
        def _make_threshold(self, pct):
            def cb(_sender):
                try:
                    ...
                    rumps.alert(title="tsamx", message=f"Couldn't set threshold: {e}")
                    return
