"""Command-line interface for Tsamx."""
...
from tsamx import __version__, paths, printer
from tsamx.exceptions import ClaudeSwitchError
from tsamx.json_output import error_envelope
...
from tsamx.settings import load_ui_settings
...
def _prog_name() -> str:
    ...
    ``python.exe C:\\Users\\me\\.local\\bin\\tsamx``). We strip that down to the
    bare command the user typed (``tsamx`` / ``tsamx``), falling back to
    ``tsamx`` for ``python -m tsamx`` and odd launchers.
    """
...
# users type `tsamx list`, `tsamx status`, `tsamx add`, etc. instead of `--list`
...
def _translate_subcommand(argv: list[str]) -> list[str]:
    ...
    exactly as before (e.g. ``tsamx switch --strategy best``, ``tsamx list --json``).
    """
    if not argv:
        return argv
...
def _run_command(argv: list[str]) -> None:
    ...
    first argument (`tsamx --debug run 2` is not supported; use
    `tsamx run 2 --debug`).
...
Examples:
  tsamx run 2
  tsamx run user@example.com
  tsamx run 2 --no-share
  tsamx run 2 --share-history
  tsamx run 2 -- --resume
        """,
    )
    parser.add_argument(
        ...
        "directory's mapping (see `tsamx map`).",
    )
    parser.add_argument(
        "--no-share",
        action="store_true",
...
def _map_command(argv: list[str]) -> None:
    """Handle `tsamx map [NUM|EMAIL] [PATH]`.
    ...
    parser = argparse.ArgumentParser(
        prog="tsamx map",
        description=(
            "Map a stored account to a directory so `tsamx run` (with no "
        ...
        epilog="""
Examples:
  tsamx map 2 ~/work/client-app
  tsamx map user@example.com          # map the current directory
  tsamx map                           # list all mappings
        """,
    )
    parser.add_argument(
        "account",
    ...
    try:
        ...
        from tsamx.mappings import MappingStore, normalize_path
...
def _unmap_command(argv: list[str]) -> None:
    """Handle `tsamx unmap [PATH]` — remove a directory→account mapping."""
    parser = argparse.ArgumentParser(
        prog="tsamx unmap",
...
def _swap_command(argv: list[str]) -> None:
    """Handle `tsamx swap NUM|EMAIL|ALIAS NUM|EMAIL|ALIAS`.
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "`tsamx list` and as numeric targets. Aliases, backups, and "
...
Examples:
  tsamx swap 1 2
  tsamx swap dev user@example.com
        """,
    )
...
def _move_command(argv: list[str]) -> None:
    """Handle `tsamx move NUM|EMAIL|ALIAS SLOT`.
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  tsamx move user@example.com 1   move an account onto shortcut 1
  tsamx move dev 1                by alias
  tsamx move 2 1                  by number (swaps if slot 1 is taken)
        """,
    )
...
def _alias_command(argv: list[str]) -> None:
    """Handle `tsamx alias [NUM|EMAIL] [NAME] [--unset]`.
...
Examples:
  tsamx alias 2 dev
  tsamx alias user@example.com dev
  tsamx alias 2 --unset
  tsamx alias                         # list all aliases
        """,
    )
    parser.add_argument(
        "account",
...
Examples:
  tsamx auto                       # foreground loop, switch at 90%% used
  tsamx auto --threshold 80        # switch earlier
  tsamx auto --model Fable         # also switch when the Fable weekly limit is hit
  tsamx auto --json                # one JSON event per line (for scripts)
  tsamx auto --once; echo $?       # single tick, outcome in exit code
  tsamx auto --dry-run             # log decisions, never actually switch
...
Defaults live in settings.json in the backup root; flags override them.
    ...
    from tsamx.autoswitch import AutoSwitchEngine, AutoSwitchEvent
    from tsamx.printer import accent, yellowed
    from tsamx.settings import load_settings, merged_with_cli
...
def _config_command(argv: list[str]) -> None:
    """Handle `tsamx config [list|get KEY|set KEY VALUE|unset KEY|path]`.
    ...
    loudly here instead of silently degrading at `tsamx auto` time.
    """
    from tsamx.settings import (
        SETTING_SPECS,
        effective_settings,
    ...
    parser = argparse.ArgumentParser(
        prog="tsamx config",
        description=(
            "Read and edit tsamx settings (settings.json in the "
            "backup root)."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
...
Examples:
  tsamx config                              # list effective settings
  tsamx config get autoswitch.threshold
  tsamx config set autoswitch.threshold 80
  tsamx config unset autoswitch.threshold   # back to the default
  tsamx config path                         # where settings.json lives
        """,
    )
    parser.add_argument(
        "--json",
    ...
    for p in (p_list, p_get):
        ...
        # pre-verb `tsamx config --json` in the shared namespace.
        p.add_argument(
            "--json",
            action="store_true",
...
def main() -> None:
    ...
    argv = sys.argv[1:]
    try:
        from tsamx.appearance import cli_should_probe, cli_theme
    ...
    # Bare `tsamx` in an interactive terminal opens the TUI dashboard (like
    # lazygit/k9s). TTY-gated on both ends so scripts and pipes keep getting
    # the usage error, and `tsamx tui` stays the explicit spelling.
    if not argv and sys.stdout.isatty() and sys.stdin.isatty():
        argv = ["--tui"]

    # Memorable subcommands (`tsamx switch <email>`, `tsamx list`, `tsamx help`, ...)
...
Commands:
  ...
  %(prog)s purge                      remove all tsamx data
...
The original flag spellings (%(prog)s --switch, %(prog)s --list, ...) keep working.
    ...
    if args.upgrade:
        from tsamx.update_check import run_self_upgrade
    ...
    try:
        ...
        if args.add_account:
            ...
            from tsamx.settings import load_settings, parse_model_names
            ...
            from tsamx.menubar import run as menubar_run
    ...
    if not args.purge and not args.upgrade and not args.json:
        from tsamx.update_check import check_for_update
