...
from tsamx import oauth, pace
from tsamx.claude_locks import claude_config_lock, claude_credentials_lock
from tsamx.json_output import (
    SCHEMA_VERSION,
    USAGE_API_KEY,
...
from tsamx.credentials import (  # noqa: F401  (constants re-exported for migrations/tests)
...
from tsamx.locking import FileLock
from tsamx.logging_config import setup_logging
from tsamx.models import (
    AccountSnapshot,
    AccountsSnapshot,
    Platform,
    SwitchTransaction,
...
from tsamx.process_detection import get_running_instances
from tsamx import poll_policy
from tsamx.settings import load_settings, parse_model_names, settings_path
from tsamx.usage_store import (
    FetchRecord,
    UsageEntry,
    UsageStore,
    with_sentinel,
...
SENTINEL_NOTES = {
    ...
    USAGE_RELOGIN_REQUIRED: "re-login needed — refresh token dead; log in with Claude Code, then run: tsamx add",
}
...
class ClaudeAccountSwitcher:
    ...
    def __init__(self, debug: bool = False):
        ...
        # Migrate legacy ~/.tsamx-backup to the new XDG path on Linux/WSL
        ...
        if migrate_legacy_backup_dir(self.backup_dir):
            ...
            print(
                f"tsamx: migrated data from {legacy} to {self.backup_dir}",
                file=sys.stderr,
            )
        ...
        from tsamx.migrations import run_migrations
    ...
    def _read_capture_credentials(self) -> str | None:
        ...
        Read-only. tsamx does not write claude's hashed keychain entry — see
        the ``session`` module docstring for why.
        """
        from tsamx.session import read_config_dir_credentials
    ...
    def _prepare_credentials_for_activation(
        ...
        tsamx does not recognize — must not leak across an account switch.
    ...
    def _post_backup_write(self, account_num: str, email: str) -> None:
        ...
        material so the next `tsamx run` re-bootstraps from this fresh backup
        ...
        if self._live_session_pids(account_num, email):
            from tsamx.session import mark_session_stale
    ...
    # -- public accessors for session mode (tsamx.session) ---------
    ...
    def set_alias(self, identifier: str, alias: str) -> tuple[str, str]:
        ...
        typo'd alias can be corrected with ``tsamx alias <old> <new>`` as
    ...
    def unset_alias(self, identifier: str) -> str:
        ...
        alias succeeds silently (no error), matching ``tsamx config unset``'s
    ...
    def swap_accounts(self, first: str, second: str) -> tuple[str, str]:
        ...
        ``sequence`` (kept sorted, so rotation and ``tsamx list`` order
                ...
                for kind, content in (("creds", creds), ("config", config)):
                    ...
                    if path.exists():
                        raise ConfigError(
                            ...
                            f"accounts still work (`tsamx list`), then delete "
    ...
    def slot_for_directory(self, directory: str | Path) -> tuple[str | None, str | None]:
        """Resolve a directory to its mapped account slot, for `tsamx run`.
    ...
    def list_mappings(self) -> None:
        """Print all directory → account mappings (for `tsamx map`)."""
        from tsamx.mappings import MappingStore

        mappings = MappingStore(self.backup_dir).all()
        if not mappings:
            print(dimmed("No directory mappings yet."))
            print(muted("Map one with: tsamx map <NUM|EMAIL> [PATH]"))
            return
        seq = self._get_sequence_data_migrated() or {}
    ...
    def switchable_account_numbers(self) -> list[str]:
        ...
        disabled (``tsamx disable``). Disabled slots stay managed and remain
        valid explicit ``tsamx switch <num|email>`` targets — they are only
    ...
    def set_account_disabled(self, identifier: str, disabled: bool) -> None:
        ...
        bare ``tsamx switch`` rotation, and the ``best`` / ``next-available``
        strategies all skip disabled slots. The account stays managed and is
        still a valid explicit ``tsamx switch <num|email>`` target, so you can
    ...
    def current_account_number(self) -> str | None:
        ...
        overwrite a login tsamx doesn't own (``_perform_switch`` would take
    ...
    def live_session_pids_for(self, account_num: str, email: str) -> list[int]:
        """Public wrapper: PIDs of live ``tsamx run`` sessions for a slot."""
    ...
    def list_unclaimed_credentials(self) -> dict[str, dict]:
        ...
        documented ``/login`` + ``tsamx add [--slot N]``.
        """
    ...
    def _session_dir(self, account_num: str, email: str) -> Path:
        from tsamx.session import session_dir_for
    ...
    def _live_session_pids(self, account_num: str, email: str) -> list[int]:
        ...
        from tsamx.session import live_sessions_for
    ...
    def _invalidate_session_credentials(self, account_num: str, email: str) -> None:
        ...
        The next `tsamx run` fails the reuse check and re-bootstraps from
        ...
        """
        from tsamx.session import STALE_MARKER, delete_macos_keychain_entry
    ...
    def _delete_session_profile(self, account_num: str, email: str) -> None:
        ...
        """
        from tsamx.session import delete_macos_keychain_entry
    ...
    def _reject_live_api_key_capture(self, creds: str) -> None:
        ...
        if looks_like_api_key(creds):
            raise ValidationError(
                ...
                "'tsamx --add-token sk-ant-api...' instead of --add-account."
            )
    ...
    def _resolve_account_identifier(self, identifier: str) -> str | None:
        ...
        raise ConfigError(
            ...
            f"Use account number instead (e.g., tsamx --switch-to 1)."
        )
    ...
    def _fetch_active_usage(
        ...
        try:
            # Lock order matches the switch path (switch_to): tsamx's own
            ...
            with (
                ...
                # Under-lock TOCTOU guards. A `tsamx switch` or `/login`
                ...
                if (
                    ...
                    if (
                        ...
                        # (contended by `tsamx switch` with a 10s acquire
            ...
            # or another tsamx operation holding the account FileLock. Either
    ...
    def _fetch_account_usage(
        ...
        # family here would log the next `tsamx run` out the same way.
        session_dir = self._session_dir(str(num), email)
        session_creds = read_session_credentials(session_dir)
        ...
        if session_creds:
            ...
            if session_oauth and session_oauth.get("accessToken"):
                ...
                # to the backup path — tsamx must not rotate the profile's
    ...
    def _select_best_switchable(
        ...
        can't be proven beneficial, it stays put; bare ``tsamx --switch``
        ...
        for num, _email, _org_name, _org_uuid, _is_active, _creds, _alias in accounts_info:
            ...
            if other:
                out.append(
                    ...
                    "account and re-add it: tsamx add --slot N"
                )
            else:
                seen[key] = snum
        return out
    ...
    def list_accounts(
        ...
        # here: users can't act on them (recovery is always /login + tsamx
    ...
    def _first_run_setup(self) -> None:
        ...
        if response.lower() == "n":
            print(dimmed("Setup cancelled. You can run 'tsamx --add-account' later."))
            return
    ...
    def switch(
        ...
        better, it stays put (run a plain ``tsamx --switch`` to rotate anyway).
        ...
        # (e.g. right after tsamx --import). Activate the recorded
            ...
            if target_disabled or not self._account_is_switchable(target):
                if target_disabled:
                    ...
                    console_reason = (
                        ...
                        f"tsamx --add-account --slot {target})"
                    )
                if json_output:
                ...
                if not fallback:
                    ...
                    raise ConfigError(
                        ...
                        "Re-add a slot with: tsamx --add-account --slot <number>"
                    )
                target = fallback
        ...
        if not self._account_exists(current_email, current_org_uuid):
            ...
            if json_output:
                ...
                return self._switch_noop(
                    ...
                    from_ref=ref,
                    to_ref=ref,
                    message="Active account is not managed; run tsamx --add-account",
                )
        ...
        # worse or unverifiable account). Bare `tsamx --switch` rotates anyway.
        if strategy == "best":
            ...
            if note == "current-unavailable":
                ...
                print(dimmed(
                    f"Current account usage is unavailable — staying on "
                    f"Account-{current_num}. Run tsamx --switch to rotate."
                ))
                return None
            ...
            if not self._account_is_switchable(candidate):
                if json_output:
                    ...
                    print(
                        ...
                        f"tsamx --add-account --slot {candidate})"
                    )
                continue
            if strategy == "next-available":
        ...
        if next_account is None:
            ...
            print(dimmed(
                ...
                "Re-add a skipped slot with: tsamx --add-account --slot <number>"
            ))
            return None
                ...
                if cur_slot == target_account and action != "reconcile":
                    ...
                    if not json_output:
                        ...
                        print(dimmed(
                            ...
                            f"tsamx --switch-to {target_account} --force"
                        ))
                        return None
                    return self._switch_noop(
                        strategy="direct",
    ...
    def _stash_live_credential(
        ...
        self._logger.warning(
            ...
            "(credentials mtime %s). Something outside tsamx rewrote the live "
    ...
    def _perform_switch(
        ...
        # Beyond tsamx's own lock, hold Claude Code's advisory locks for the
        ...
        with FileLock(self.lock_file), claude_credentials_lock(), claude_config_lock():
            ...
            # (e.g. right after import), tsamx has no tracked active
            ...
            if force_activate or current_identity is None or current_account is None:
                ...
                # unknown to tsamx); a numbered ref when --force ran with a
                # managed live login.
                ...
                if not target_creds:
                    raise SwitchError(
                        ...
                        f"Re-add with: tsamx --add-account --slot {target_account}"
                    )
                ...
                if kind in ("foreign", "alien", "known-foreign"):
                    ...
                    if kind == "foreign":
                        msg = (
                            ...
                            "in as it and run: tsamx add --slot "
                            f"{foreign_slot}"
                        )
                        ...
                        msg = (
                            ...
                            "account, log in as it and run: tsamx add"
                        )
                    if emit_output:
                        warning(msg)
                    else:
                    ...
                    # Untouched since tsamx wrote it — the slot already holds
        ...
        if emit_output:
            ...
            try:
                ...
                print(dimmed("  (usage display unavailable — run `tsamx --list` to retry)"))
            print()
            self._print_switch_followup()
            print()
        self._replan_new_active(
    ...
    def purge(self) -> None:
        """Remove all traces of tsamx from the system.
        ...
        - The active backup directory (XDG path on Linux/WSL, ~/.tsamx-backup elsewhere)
        - Any stale legacy ~/.tsamx-backup directory left around from
        ...
        warning("This will remove ALL tsamx data from your system:")
        ...
        if removed_items:
            ...
            print(f"\n{dimmed('No tsamx data found to remove.')}")
