"""Directory → account mappings for `tsamx run` auto-resolution.

Maps a normalized absolute directory path to a stored account identity
(email + organizationUuid). `tsamx run` with no account argument resolves the
...
from tsamx.models import get_timestamp
