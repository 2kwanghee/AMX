...
lock) make it safe to run alongside an external ``tsamx auto``.
...
from tsamx.settings import SETTING_SPECS, load_settings, parse_model_names
from tsamx.tui import data
from tsamx.tui.modals import ConfirmModal
from tsamx.tui.theme import Palette
    ...
    def action_toggle_live(self) -> None:
        ...
        if self._engine.dry_run:
            self.app.push_screen(
                ConfirmModal(
                    "Go live? tsamx will switch your active account "
                    "automatically when the threshold is reached.\n\n"
                    "(Same behavior as running `tsamx auto` in a terminal.)",
                    title="Go live",
                    yes_label="Go live",
                ),
