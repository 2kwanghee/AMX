...
Mirrors claude-code's own resolution so tsamx reads and writes the same files
...
Also resolves the tsamx backup root, which on Linux/WSL follows the XDG Base
Directory Specification (``$XDG_DATA_HOME/tsamx``) and falls back to the
legacy ``~/.tsamx-backup`` on macOS/Windows.
...
LEGACY_BACKUP_DIRNAME = ".tsamx-backup"
...
def get_legacy_backup_root() -> Path:
    """Return the legacy (pre-XDG) backup root: ``~/.tsamx-backup``."""
    return Path.home() / LEGACY_BACKUP_DIRNAME


def get_backup_root() -> Path:
    """Return the tsamx backup root for the current platform.

    Linux/WSL: ``$XDG_DATA_HOME/tsamx`` (default ``~/.local/share/tsamx``).
    macOS/Windows/unknown: ``~/.tsamx-backup`` (legacy layout).
    ...
    if Platform.detect() in (Platform.LINUX, Platform.WSL):
        ...
        if xdg:
            ...
            if xdg_path.is_absolute():
                return xdg_path / "tsamx"
        return Path.home() / ".local" / "share" / "tsamx"
    return get_legacy_backup_root()


# Names that any prior tsamx run may have created in the backup root without
...
def migrate_legacy_backup_dir(target: Path) -> bool:
    ...
    * No flag, both paths exist → genuine collision, refuse — *unless* the
      ...
      prior tsamx run may have laid down before legacy reappeared (e.g.
