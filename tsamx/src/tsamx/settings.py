...
One versioned JSON file for user-tunable tsamx preferences, written
...
class AutoSwitchSettings:
    """Policy knobs for the auto-switch engine (``tsamx auto``).
...
class SettingSpec:
    ...
    (`_clamped`) and the strict validation in `tsamx config set`
...
SETTING_SPECS: dict[str, SettingSpec] = {
    ...
    for spec in (
        ...
        SettingSpec(
            ...
            help="Poll interval for the tsamx auto loop, in seconds",
        ),
        SettingSpec(
...
def parse_setting_value(spec: SettingSpec, raw_value: str):
    """Strictly parse a CLI-provided string for `tsamx config set`.
    ...
    not by silently degraded behavior at `tsamx auto` time.
    """
    if spec.kind == "bool":
    ...
    if spec.kind == "string":
        ...
        if not value:
            raise ConfigError(
                ...
                f"'tsamx config unset {spec.dotted}' to clear it"
            )
        return value
    try:
...
def set_setting(backup_root: Path, dotted_key: str, raw_value: str):
    """Validate and persist one key for `tsamx config set`; returns the value.
...
def effective_settings(backup_root: Path) -> list[tuple[SettingSpec, object, bool]]:
    ...
    to the default still counts — so `tsamx config`'s "(default)" marker
...
def atomic_write_json(path: Path, data: dict) -> None:
    ...
    **Writes THROUGH a symlink, never over it.** A rename swaps a directory
    ...
    - The 0700 hardening stays on the directory tsamx owns. Applying it to
