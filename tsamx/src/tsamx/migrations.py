"""One-time, run-once data migrations for tsamx.

A small, boring home for compatibility migrations so they don't pollute the
core switch/read/write flow in :mod:`tsamx.switcher`. Each migration:
...
- raises :class:`~tsamx.exceptions.MigrationIncomplete` (or any other
...
from tsamx.exceptions import MigrationIncomplete
from tsamx.fsutil import replace_with_retry
from tsamx.models import Platform, get_timestamp
from tsamx.switcher import KEYRING_SERVICE, SECURITY_SERVICE
...
def migrate_windows_keyring_to_files(switcher: "ClaudeAccountSwitcher") -> bool:
    ...
    if migrated:
        print(
            f"tsamx: migrated {migrated} Windows credential(s) from "
...
def migrate_macos_keyring_to_security(switcher: "ClaudeAccountSwitcher") -> bool:
    ...
    :mod:`tsamx.macos_keychain`) instead of the third-party ``keyring``
    ...
    if migrated:
        print(
            f"tsamx: migrated {migrated} macOS credential(s) from the keyring "
