...
def list_tenants(db: DbSession, pageSize: PageSize = 50, pageToken: PageToken = None):  # noqa: N803
...
def create_tenant(body: schemas.TenantCreate, db: DbSession):
...
def get_tenant(tenant_id: uuid.UUID, db: DbSession):
...
def update_tenant(tenant_id: uuid.UUID, body: schemas.TenantUpdate, db: DbSession):
...
def delete_tenant(tenant_id: uuid.UUID, db: DbSession) -> Response:
