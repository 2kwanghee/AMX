...
def _reserve_due(db: Session) -> list[dict]:
    """만기 행을 리스 예약하고 발송에 필요한 스냅샷 dict 목록을 돌려준다.

    락을 잡고, ``next_attempt_at <= now`` 행을 ``FOR UPDATE SKIP LOCKED``로 집어
    ``next_attempt_at``를 리스 창만큼 앞으로 밀고 커밋한다(락 해제). 커밋 후에는 이
    행들이 만기에서 빠지므로 발송을 락 밖에서 안전하게 진행할 수 있다.
    """
...
def _finalize(db: Session, delivered: list[uuid.UUID], failed: list[dict]) -> None:
    """성공 행은 삭제, 실패 행은 백오프 재설정 또는 상한 초과 시 폐기. caller와 무관한 짧은 txn."""
    now = _now()
    if delivered:
        db.execute(
            delete(AlertWebhookOutbox).where(AlertWebhookOutbox.id.in_(delivered))
        )
    discard: list[uuid.UUID] = []
    for row in failed:
        attempt = row["attempt"] + 1
        if attempt >= _MAX_ATTEMPTS:
            discard.append(row["id"])
            _logger.warning(
    ...
    if discard:
        db.execute(delete(AlertWebhookOutbox).where(AlertWebhookOutbox.id.in_(discard)))
    db.commit()
