...
# Written by usage_cost.sweep_snapshot_retention: the exclusive upper bound of the
# most recent retention delete. Both sweeps' G27 rewind clamp read it so a rewind
# can never drop a cursor below already-purged data — re-aggregating a sealed day
# from the snapshots that happened to survive the purge would overwrite its sealed
# row with a partial value (adversary regression). Defined here — the module both
# usage_cost and billing already share (usage_cost imports billing, not vice
# versa) — to keep one source of truth without an import cycle. Monotonic: the
# retention writer never moves it backward.
PURGE_KIND = "snapshot_purge"
...
def sweep_billing(db: Session) -> int:
    ...
    if start > last_closed_end:
        ...
        # re-bills the reopened days — the per-day insert is ON CONFLICT DO NOTHING
        # on the (tenant, kind, period_start) anchor, so no double-bill.
        #
        ...
        # rows. Rewinding below such a day would let the next tick re-aggregate it
        # from the afternoon survivors and overwrite the sealed row with a partial
        # value. Keeping the cursor at/above the purged bound leaves those days
        # sealed; the surviving snapshots below the cursor are surfaced by the
        # below-watermark warning above rather than silently re-billed.
        purge = db.get(BillingCursor, PURGE_KIND)
        ...
        db.execute(
            ...
            .values(kind=KIND, watermark=rewind_to, updated_at=now)
        ...
        # days for partial re-aggregation AND make this early lock release unsafe.
        db.commit()
        _logger.warning(
